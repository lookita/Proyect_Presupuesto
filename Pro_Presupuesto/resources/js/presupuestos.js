document.addEventListener('DOMContentLoaded', () => {
    ...
    // Datos de productos de ejemplo (en el futuro, esto vendrá de una API de Laravel)
    const productos = [
        { id: 1, nombre: 'Servicio de Diseño Gráfico', precio: 1500.00 },
        { id: 2, nombre: 'Desarrollo Web (por hora)', precio: 50.00 },
        { id: 3, nombre: 'Licencia de Software (anual)', precio: 300.00 },
        { id: 4, nombre: 'Consultoría SEO', precio: 800.00 },
    ];
    ...
    function agregarFilaProducto() {
        ...
        row.innerHTML = `
            <td class="py-2 px-4">
                <select name="productos[][id]" class="w-full px-3 py-2 border rounded-md producto-select focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">Selecciona un producto...</option>
                    ${productos.map(p => `<option value="${p.id}">${p.nombre}</option>`).join('')}
                </select>
            </td>
            <td class="py-2 px-4">
                <input type="number" name="productos[][cantidad]" class="w-full px-3 py-2 border rounded-md cantidad-input text-right focus:outline-none focus:ring-2 focus:ring-blue-500" value="1" min="1">
            </td>
        ...
        select.addEventListener('change', (e) => {
            ...
            if (productoSeleccionado) {
                row.querySelector('.precio-unitario').textContent = `$${productoSeleccionado.precio.toFixed(2)}`;
            } else {
    ...
    // Agregamos una fila inicial al cargar la página
    agregarFilaProducto();
});